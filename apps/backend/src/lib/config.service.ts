import { type ConfigKey, ConfigKeyEnum } from "@repo/zod-types"

import { configRepo } from "../db/repositories/config.repo"

export const configService = {
  async isSignupDisabled(): Promise<boolean> {
    const config = await configRepo.getConfig(ConfigKeyEnum.enum.DISABLE_SIGNUP)
  ...
  async getSessionLifetime(): Promise<number | null> {
    ...
    return Number.isNaN(lifetime) ? null : lifetime
  },
