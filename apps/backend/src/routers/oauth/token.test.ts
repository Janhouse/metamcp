  ...
  return {
    oauthRepository: {
      ...
      setAccessToken: vi.fn().mockImplementation((token, data) => {
        store.set(token, { ...data, access_token: token, created_at: new Date() });
        if (data.refresh_token) {
          refreshIndex.set(data.refresh_token, token);
        }
      }),
      getAccessToken: vi.fn().mockImplementation((token) => store.get(token) ?? null),
      deleteAccessToken: vi.fn().mockImplementation((token) => {
        const data = store.get(token);
        if (data?.refresh_token) refreshIndex.delete(data.refresh_token);
        store.delete(token);
      }),
      getAccessTokenByRefreshToken: vi.fn().mockImplementation((refreshToken) => {
        const accessToken = refreshIndex.get(refreshToken);
        if (!accessToken) return null;
        return store.get(accessToken) ?? null;
      }),
      deleteAccessTokenByRefreshToken: vi.fn().mockImplementation((refreshToken) => {
        const accessToken = refreshIndex.get(refreshToken);
        if (accessToken) {
...
vi.mock("./utils", () => {
  ...
  return {
    ...
    generateSecureRefreshToken: vi.fn(() => `mcp_refresh_test_${++refreshCounter}`),
...
describe("OAuth refresh token support", () => {
  ...
  describe("generateSecureRefreshToken", () => {
    it("should generate tokens with mcp_refresh_ prefix", async () => {
      const { generateSecureRefreshToken } = await vi.importActual<any>("./utils");
    ...
    it("should generate unique tokens", async () => {
      const { generateSecureRefreshToken } = await vi.importActual<any>("./utils");
      const tokens = new Set(Array.from({ length: 100 }, () => generateSecureRefreshToken()));
  ...
  describe("repository - getAccessTokenByRefreshToken", () => {
    it("should find token data by refresh token", async () => {
      ...
      const result = await oauthRepository.getAccessTokenByRefreshToken("mcp_refresh_1");
    ...
    it("should return null for unknown refresh token", async () => {
      const result = await oauthRepository.getAccessTokenByRefreshToken("mcp_refresh_unknown");
  ...
  describe("repository - deleteAccessTokenByRefreshToken", () => {
    it("should delete the entire row when refresh token is revoked", async () => {
      ...
      const byRefresh = await oauthRepository.getAccessTokenByRefreshToken("mcp_refresh_1");
  ...
  describe("token rotation", () => {
    it("should issue new tokens when refresh token is exchanged", async () => {
      ...
      const oldData = await oauthRepository.getAccessTokenByRefreshToken("mcp_refresh_1");
      ...
      const oldLookup = await oauthRepository.getAccessTokenByRefreshToken("mcp_refresh_1");
      expect(oldLookup).toBeNull();

      // New refresh token should work
      const newLookup = await oauthRepository.getAccessTokenByRefreshToken("mcp_refresh_2");
