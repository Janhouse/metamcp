...
const checkServerErrorStatus = async (serverUuid: string): Promise<boolean> => {
  try {
    ...
    logger.error(`Error checking server error status for ${serverUuid}:`, error)
    return false
  }
}
...
const createTransport = async (req: express.Request): Promise<Transport> => {
  ...
  if (transportType === McpServerTypeEnum.enum.STDIO) {
    ...
    logger.info(`SSE transport: url=${url}, headers=${JSON.stringify(headers)}`)
...
serverRouter.delete("/mcp", async (req, res) => {
  ...
  if (sessionId) {
    try {
      ...
      logger.info(`Session ${sessionId} terminated and cleaned up successfully`)
      res.status(200).end()
    } catch (error) {
