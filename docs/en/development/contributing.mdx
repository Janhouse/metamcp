...
- **Bun** (1.4+, the Rust build) and **Node.js 20+** installed
...
- ✅ **Fix liniting as much as possible (somewhat tolerant as we dev rapidly)** (`bun run lint`)
...
<AccordionGroup>
  ...
  <Accordion icon="flask-conical" title="Testing OIDC">
    **For OIDC development:**
    ...
    2. Configure redirect URI: `${APP_URL}/api/auth/callback/oidc`
...
<AccordionGroup>
  <Accordion icon="database" title="Schema Migrations">
    **Creating migrations:**
    ...
    # Apply migrations
    bun run db:migrate:dev # which uses env.local for PG related env vars
