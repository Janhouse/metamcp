...
- **Bun**（1.4+，Rust 版本）和 **Node.js 20+** 已安装
...
- ✅ **代码遵循项目标准**
- ✅ **尽可能修复代码检查问题（在快速开发时相对宽容）** (`bun run lint`)
- ✅ **没有 TypeScript 错误**
- ✅ **如需要，文档已更新**
- ✅ **手动测试了更改**
- ✅ **如需要，包含数据库迁移**
...
<AccordionGroup>
  ...
  <Accordion icon="flask-conical" title="测试 OIDC">
    **OIDC 开发：**
    ...
    2. 配置重定向 URI：`${APP_URL}/api/auth/callback/oidc`
    3. 测试身份验证流程
    4. 验证数据库中的用户创建
    5. 启用调试日志进行故障排除
  </Accordion>
...
<AccordionGroup>
  <Accordion icon="database" title="架构迁移">
    **创建迁移：**
    ...
    # 应用迁移
    bun run db:migrate:dev # 使用 env.local 中的 PG 相关环境变量
