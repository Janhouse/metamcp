...
> **Upgrading across better-auth 1.7:** no IdP-side change is needed. The
> redirect URI generated by 1.7.4 is still
> `${APP_URL}/api/auth/oauth2/callback/${OIDC_PROVIDER_ID}`, the pre-1.7 value —
> verified by reading `redirect_uri` out of the authorize URL that
> `POST /api/auth/sign-in/social` returns. (`/api/auth/callback/:id` is also
> routed and reaches the same handler, but is not what gets sent to the IdP.)
> Accounts are still keyed on `(providerId, accountId)` too: 1.7.0–1.7.2 briefly
> re-keyed them on `(issuer, accountId)` and 1.7.3 reverted it, so migrations
> `0017`/`0018` add and then drop the short-lived `accounts.issuer` column.
